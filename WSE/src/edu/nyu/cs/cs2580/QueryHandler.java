...
class QueryHandler implements HttpHandler {
	...
	public static class CgiArguments {
		...
		// for query extention part in hw3
		private int numDocs = 10;

		// for query extention part in hw3
		private int numTerms = 100;
		...
		public CgiArguments(String uriQuery) {
			...
			for (String param : params) {
				...
				if (key.equals("query")) {
					...
					try {
						this.numDocs = Integer.parseInt(val);
					} catch (IllegalArgumentException e) {
						// Ignored, search engine should never fail upon invalid
						// user input.
					}
					...
					try {
						this.numTerms = Integer.parseInt(val);
					} catch (IllegalArgumentException e) {
						// Ignored, search engine should never fail upon invalid
						// user input.
					}
	...
	private void constructTextOutput(final Vector<ScoredDocument> docs,
		...
		if(docs.size() == 0) {
			...
			response.append("No match results");
		}
		for (ScoredDocument doc : docs) {
			response.append(response.length() > 0 ? "\n" : "");
			response.append(doc.asTextResult()).
			append("\tPage Rank:" + doc.getDocument().getPageRank()).
			append("\tNumviews:" + doc.getDocument().getNumViews()).append("\n");
		}
		response.append(response.length() > 0 ? "\n" : "");
	}
	...
	public void handle(HttpExchange exchange) throws IOException {
		...
		if (uriPath.equals("/prf")) {
			...
			Vector<ScoredDocument> scoredDocs = ranker.runQuery(processedQuery,
					cgiArgs.numDocs);
			String result = queryExpansion.runExpansion(scoredDocs,cgiArgs.numTerms,cgiArgs._query);
